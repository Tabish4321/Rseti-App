    ...
    */
        private var settleStatusList: List<SettlementStatus> = emptyList()
    ...
    //salary
        ...
        private fun collectStatusResponse() {
            lifecycleScope.launch {
                commonViewModel.getSettleStatusApi.collectLatest { resource ->
                    when (resource) {
                        is Resource.Loading -> {
                        ...
                        is Resource.Error -> {
                            ...
                            Toast.makeText(requireContext(), "Failed to fetch settlement status", Toast.LENGTH_SHORT).show()
                        }
                        ...
                        is Resource.Success -> {
                            ...
                            resource.data?.let { response ->
                                if (response.responseCode == 200) {
                                    // Clear and update your local status list
                                    settleStatusList = response.wrappedList

                                    val statusNames = settleStatusList.map { it.status }
                                    ...
                                    spinnerStatus.setAdapter(settleStatusAdapter)

                                } else if (response.responseCode == 301 || response.responseCode == 302) {
                                    Toast.makeText(requireContext(), response.responseMsg ?: "No statuses found", Toast.LENGTH_SHORT).show()
